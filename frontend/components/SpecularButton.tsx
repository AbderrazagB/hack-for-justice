...
export interface SpecularButtonProps {
  ...
  /**
   * Render an anchor instead of a button. Added for Sahilli: most primary
   * actions navigate, and a button that pushes a route is not a link.
   */
  href?: string;
}
...
const SpecularButton = ({
  ...
  // Shared by both render paths; the WebGL layer only reads geometry.
  const btnRef = useRef<HTMLElement>(null);
  const fxRef = useRef<HTMLSpanElement>(null);
  const propsRef = useRef<ShaderProps>({} as ShaderProps);

  // Upstream assigned this during render, which React forbids. Syncing it in
  // an effect keeps the shader reading fresh props without a render-phase write.
  useEffect(() => {
