...
export function SubmissionReview({ submission }: { submission: Submission }) {
  ...
  async function decide(action: ReviewAction) {
    ...
    try {
      ...
      setNote("");
      // Re-render the server component so status, history and the queue's
      // figures all reflect the decision.
  ...
  const flaggedHere = submission.flags.some((flag) =>
  ...
  return (
    <div className="min-h-screen">
      <Header
        trailing={
          <Link href="/admin" className="text-sm font-semibold text-[var(--brand)]">
            ← Retour à la file
          </Link>
        }
      />
        ...
        <div className="mt-10">
          ...
          {document ? (
            <div className="mt-4 grid gap-4 lg:grid-cols-2">
              ...
              <FieldTable document={document} flagged={flaggedHere} />
            </div>
          ) : (
            <div className="mt-4">
